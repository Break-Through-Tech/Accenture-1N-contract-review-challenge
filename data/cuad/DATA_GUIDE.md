...
| `answers[].answer_start` | integer | Zero-based character position of `text` in its parent `context`. | `430` |

For every answer, the basic integrity check is:
...
risk labels; any such score must come from a separate, documented project rule or
review process.
...
The second span connects that commencement date to when the agreement becomes
...
and a later punctuation variant. Preserve all answer spans when training or
evaluating extraction. Slicing `context` at each of the five offsets reproduces its
